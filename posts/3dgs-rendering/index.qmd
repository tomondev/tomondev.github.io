---
title: "Spherical Harmonics and 3D Gaussian Splatting Rendering"
date: 2025-09-01
author: Tom
draft: true
---
...
Notice that SH are complex-valued functions. Using the definition above, we derive $Y_l^m$ up to degree $l=3$.
...
Complex-valued functions are not yet usable in our applications. The real basis of SH is defined as follows:
$$
Y_{\ell}^{m}(\theta, \varphi)=\begin{cases} \sqrt{2}(-1)^{m} \operatorname{Im}[Y_{\ell}^{|m|}] & \text{if } m<0 \\ Y_{\ell}^{0} & \text{if } m=0 \\ \sqrt{2}(-1)^{m} \operatorname{Re}[Y_{\ell}^{m}] & \text{if } m>0 \end{cases}
$$
Here are the visualizations of the real SH for different $l$ and $m$ values. Red and blue represent positive and negative areas, respectively.
...
The above visualizations are created by the Colab: [![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1h6gI3gn1ZXvsNCQYyDJ0DxFXXwH0VowB).

# 3DGS Rendering

The original [3DGS paper](https://arxiv.org/abs/2308.04079) use SH of degree 2 ($l=2$), which requires 9 coefficients per color channel for a total of 27 SH coefficients per Gaussian. Generally, given a viewing angle $(\theta, \varphi)$, coefficients $k_l^m$, the color can be determined by
$$
c(\theta,\varphi,k_l^m)=\sigma\left(\sum_{l=0}^2\sum_{m=-2}^2 k_l^m Y_{l}^{m}(\theta, \varphi)\right)
$$
where $\sigma$ is the sigmoid function. Note that we take the real part of the SH.
